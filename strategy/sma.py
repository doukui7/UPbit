...
class SMAStrategy:
    ...
    │ 거래소       │ 매매 수수료   │ 원화 출금     │ 비고                  │
    ├─────────────┼──────────────┼──────────────┼──────────────────────┤
    │ 업비트       │ 0.05%        │ 1,000원      │ 국내 거래량 1위        │
    │ 빗썸        │ 0.04%        │ 1,000원      │ 쿠폰 적용 시 할인      │
    │ 코인원       │ 0.20%        │ 1,000원      │ 등급별 할인 가능       │
    │ 고팍스       │ 0.20%        │ 1,000원      │                      │
    │ 바이낸스     │ 0.10%        │ -            │ BNB 사용 시 0.075%    │
    └─────────────┴──────────────┴──────────────┴──────────────────────┘

    * 수수료 = 매수/매도 각각 적용 (왕복 시 2배)
    * 업비트 기준 100만원 매매 시: 매수 500원 + 매도 500원 = 1,000원
    * 코인 출금 수수료는 코인/네트워크별 상이 (BTC ≈ 0.0005 BTC 등)

    [세금]
    * 2025년 기준 가상자산 소득세 과세 유예 중 (2027년까지)
    * 향후 시행 시: 기본공제 250만원 초과분에 대해 22% (지방세 포함)
    """
    def __init__(self):
        pass
