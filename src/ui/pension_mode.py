...
from src.ui.components.pension_orders import (
    load_pen_orders,
    add_pen_order,
    update_pen_order_status,
    execute_pending_pen_orders,
    render_pension_orders_tab,
)


# 예약 주문 데이터 관리: src/ui/components/pension_orders.py로 이동
# 호환성 별칭
_load_pen_orders = load_pen_orders
_add_pen_order = add_pen_order
_update_pen_order_status = update_pen_order_status
_execute_pending_pen_orders = execute_pending_pen_orders
...
"""
    ...
    # ══════════════════════════════════════════════════════════════
    with tab_p_orders:
        # etf_options 구축 (tab_p3와 동일 로직)
        _po_etf_codes = []
        _po_etf_codes.extend([kr_iwd, kr_gld, kr_ief, kr_qqq, kr_shy])
        if _dm_settings:
            _dm_map_po = _dm_settings.get("kr_etf_map", {}) or {}
            _po_etf_codes.extend([_dm_map_po.get("SPY", ""), _dm_map_po.get("EFA", ""), _dm_map_po.get("AGG", "")])
        if _vaa_settings:
            _vaa_map_po = _vaa_settings.get("kr_etf_map", {}) or {}
            _po_etf_codes.extend([str(v) for v in _vaa_map_po.values() if str(v).strip()])
        if _cdm_settings:
            _cdm_map_po = _cdm_settings.get("kr_etf_map", {}) or {}
            _po_etf_codes.extend([str(v) for v in _cdm_map_po.values() if str(v).strip()])
        _po_etf_codes = [str(c).strip() for c in _po_etf_codes if str(c).strip()]
        _po_etf_codes = list(dict.fromkeys(_po_etf_codes))
        if not _po_etf_codes:
            _po_etf_codes = ["360750"]
        _po_etf_options = {_fmt_etf_code_name(c): c for c in _po_etf_codes}
