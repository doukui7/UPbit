...
def _default_progress() -> list:
    ...
    return [
        ...
        {"phase": "1. 인프라 구축", "task": "GH Actions 워크플로우 구성", "status": "진행중", "note": "coin_trade.yml 스케줄 조건 수정, 실행 검증 대기"},
        {"phase": "2. 보안 설정", "task": "API 키 환경변수 분리 (.env)", "status": "완료", "note": "GitHub Secrets + VM .env"},
        {"phase": "2. 보안 설정", "task": "텔레그램 알림 봇 연동", "status": "진행중", "note": "코드 구현 완료, 실행 후 알림 수신 검증 대기"},
        ...
        {"phase": "3. 매매 엔진", "task": "시그널 전환 감지 (signal_state.json)", "status": "진행중", "note": "코드 구현 완료, 실제 전환 저장 검증 대기"},
        {"phase": "3. 매매 엔진", "task": "자동매매 실행 (run_auto_trade)", "status": "진행중", "note": "코드 구현 완료, GH Actions 스케줄 수정 후 검증 대기"},
        {"phase": "3. 매매 엔진", "task": "자동매매 스케줄러 (Python 단일 로직)", "status": "진행중", "note": "vm_scheduler.py 구현 완료, VM 상주 실행 검증 대기"},
        {"phase": "3. 매매 엔진", "task": "수동 주문 기능 (manual_order)", "status": "진행중", "note": "코드 구현 완료, UI→GH Actions→VM 경로 검증 대기"},
        {"phase": "3. 매매 엔진", "task": "주문 로그 기록 (trade_log.json)", "status": "진행중", "note": "코드 구현 완료, 파일 생성 및 기록 검증 대기"},
        {"phase": "4. 대시보드", "task": "Streamlit 메인 UI 구성", "status": "완료", "note": "코인/골드/ISA/연금저축 4모드"},
        {"phase": "4. 대시보드", "task": "실시간 잔고/시세 표시", "status": "진행중", "note": "account_sync 연동 수정, 실시간 갱신 검증 대기"},
        {"phase": "4. 대시보드", "task": "전략 트리거 및 주문 로그 탭", "status": "진행중", "note": "코드 구현 완료, trade_log.json 동기화 검증 대기"},
        {"phase": "4. 대시보드", "task": "프로젝트 로직 탭", "status": "완료", "note": "시스템 문서 + 진행 내역"},
        {"phase": "5. 운영 관리", "task": "스케줄링 단일 로직 (Python 스케줄러)", "status": "진행중", "note": "vm_scheduler.py 구현 완료, VM 상주 실행 검증 대기"},
        {"phase": "5. 운영 관리", "task": "헬스체크 오탐 방지", "status": "진행중", "note": "코드 구현 완료, 정상 동작 검증 대기"},
        ...
        {"phase": "6. ISA/연금저축", "task": "연금저축 예약주문 실행", "status": "진행중", "note": "pension_orders.json 로직 변경 완료, 실행 검증 대기"},
    ]
...
""")
    ...
    issues = pd.DataFrame([
        {"우선순위": "🟢 해결", "이슈": "이중 스케줄링 (cron + scheduler) 충돌",
         "상태": "코드수정완료", "조치": "Python 스케줄러를 유일한 실행 주체로 통합, cron은 watchdog 전용"},
        {"우선순위": "🟢 해결", "이슈": "헬스체크 오탐 (상태파일 미갱신)",
         "상태": "코드수정완료", "조치": "vm_run_job.sh에 상태파일 기록 추가 + 로그 mtime 보조 확인"},
        {"우선순위": "🟢 해결", "이슈": "signal_state.json git reset 덮어쓰기",
         "상태": "코드수정완료", "조치": "모든 SSH 스크립트에 백업/복원 패턴 적용 (2026-03-06)"},
        {"우선순위": "🟢 해결", "이슈": "account_sync에서 signal_state 미동기화",
         "상태": "코드수정완료", "조치": "SCP + git add에 signal_state.json + trade_log.json 포함"},
        {"우선순위": "🟢 해결", "이슈": "연금저축 GH Actions 스케줄 누락",
         "상태": "코드수정완료", "조치": "pension_trade.yml에 cron 스케줄 + SCP push 스텝 추가"},
        {"우선순위": "🔴 긴급", "이슈": "coin_trade.yml trade job이 schedule에서 미실행",
         "상태": "코드수정완료", "조치": "trade job if 조건에 schedule 이벤트 추가 (2026-03-06). 다음 스케줄 실행 검증 대기"},
        {"우선순위": "🟡 중간", "이슈": "보충 매수/매도 1시 미작동",
         "상태": "원인분석완료", "조치": "coin_trade.yml trade job 미실행이 원인. 스케줄 조건 수정 후 다음 실행 검증 대기"},
        {"우선순위": "🟡 중간", "이슈": "실시간 잔고 표시 미동작",
         "상태": "코드수정완료", "조치": "account_sync GH Actions 연동 수정. 검증 대기"},
